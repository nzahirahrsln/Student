...
namespace HighMonth1ExpsData
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string query = "SELECT TOP 1 [Given Name] + ' (' + CAST([Expense (Month 1)] AS VARCHAR(10)) + ')' AS [Highest Month 1 Expenses]" +
                                "FROM Table1 INNER JOIN Table2 ON Table1.Userid = Table2.Userid INNER JOIN Table3 ON Table2.Col_1 = Table3.Col_1 " +
                                "ORDER BY[Expense(Month 1)] DESC";
