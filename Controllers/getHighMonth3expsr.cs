...
namespace HighMonth3ExpsData
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string query = "SELECT TOP 1 [Given Name] + ' (' + CAST([Expense (Month 3)] AS VARCHAR(10)) + ')' AS [Highest Month 3 expenses] " +
                                "FROM Table1 t1 INNER JOIN Table2 t2 ON t1.Userid = t2.Userid INNER JOIN Table3 t3 ON t2.Col_1 = t3.Col_1 " +
                                "ORDER BY[Expense(Month 3)] DESC";
